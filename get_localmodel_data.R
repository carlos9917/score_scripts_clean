####################################################################
#version for nea, and hopefully other local models 
get_localmodel_data <- function(model,files)
{
fctot <- NULL
fc <- NULL
cat("Reading local model ",model,"\n")
#print(length(files[[model]]))
        for (ii in 1:length(files[[model]])) {
            infile <- files[[i]][ii]
            m <- paste(infile,sep="")            
	    cat("reading  ",m, "\n")
            x <- readLines(m)
